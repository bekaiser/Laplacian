...
! FILE:         mpi_scatter_gather_1D.f90
! AUTHOR:       Bryan Kaiser
! DATE:         6/30/17
! DESCRIPTION:  A 2D array is split into 4 1D vectors that are scattered to 
!               4 ranks, where 1 is added to each value of each vector. Then 
!               the vectors on each rank are gathered back to the an array 
!               on the master rank.
...
! ****************************************************************************
program mpi_scatter_gather_1D
include 'mpif.h'

integer, parameter :: Np = 4, Nx = 4, Ny = 5
integer :: i, j, Nprocs, rank, ierror
real*8, dimension(Ny,Nx) :: u, ug  ! double precision
real*8, dimension(Ny) :: u0        ! double precision
integer, dimension(Np) :: uc = Ny 
...
if ( rank == 0 ) then
  ...
  us(2) = 5 ! the second rank vector starts with the address: u(1) + us(2) = 6
  us(3) = 10 ! the third rank vector starts with the address: u(1) + us(3) = 11
  us(4) = 15 ! the fourth rank vector starts with the address: u(1) + us(4) = 16
end if
...
if ( Nprocs == Np ) then 
  ...
  call MPI_SCATTER( u, Ny, MPI_DOUBLE, u0, Ny, MPI_DOUBLE, 0, MPI_COMM_WORLD, ierror)
  !call MPI_SCATTERV( u, uc, us, MPI_DOUBLE, u0, Ny, MPI_DOUBLE, 0, MPI_COMM_WORLD, ierror)
  ...
  do i = 0,Nprocs-1 ! looping over processes
    if ( rank == i ) then
      ...
      !print *, int(u0)
      do j = 1,Ny
        u0(j) = u0(j) + 1.0 ! add 1 to each value
      end do
    end if 
  end do 
 ...
 call MPI_GATHER( u0, Ny, MPI_DOUBLE, ug, Ny, MPI_DOUBLE, 0, MPI_COMM_WORLD, ierror)
 !call MPI_GATHERV( u0, Ny, MPI_DOUBLE, ug, uc, us, MPI_DOUBLE, 0, MPI_COMM_WORLD, ierror)
...
end program mpi_scatter_gather_1D
